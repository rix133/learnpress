...
if ( ! function_exists( 'learn_press_course_price' ) ) {
	...
	function learn_press_course_price() {
		...
		if ( $user && $user->get_course_access_level( $course->get_id() ) >= 50 ) {
			return;
		}
...
if ( ! function_exists( 'learn_press_course_item_content' ) ) {
	...
	function learn_press_course_item_content() {
		...
		if ( $item->is_blocked() ) {
			...
			if ( $lp_course->get_meta( '_lp_submission' ) === 'yes' ) {
				?>
                <div class="item-login-register-form"></div><?php
			} else {
...
if ( ! function_exists( 'learn_press_content_item_access_forbidden' ) ) {
	function learn_press_content_item_access_forbidden() {
...
if ( ! function_exists( 'learn_press_content_item_edit_links' ) ) {
	...
	function learn_press_content_item_edit_links() {
		...
		if ( $lp_course_item && ( $post_type_object = get_post_type_object( $lp_course_item->get_item_type() ) )
			...
			$type = learn_press_get_post_type( $lp_course_item->get_id() );
		...
		if ( $lp_quiz_question ) {
			if ( ( $post_type_object = get_post_type_object( $lp_quiz_question->get_item_type() ) )
				...
				$type = learn_press_get_post_type( $lp_quiz_question->get_id() );
				$wp_admin_bar->add_menu( array(
					'id'    => 'edit-' . $type,
					'title' => $post_type_object->labels->edit_item,
					'href'  => $edit_post_link
...
if ( ! function_exists( 'learn_press_course_class' ) ) {
	...
	function learn_press_course_class( $classes, $class, $post_id = '' ) {
		...
		if ( ! $post_id || 'lp_course' !== learn_press_get_post_type( $post_id ) ) {
			return $classes;
		}
		$classes[] = 'course';
...
function learn_press_setup_query_courses( $wp_query ) {
	if ( $wp_query->posts ) {
		$course_ids = wp_list_pluck( $wp_query->posts, 'ID' );
		LP_Helper_CURD::cache_posts( $course_ids );
...
add_action( 'loop_start', 'learn_press_setup_query_courses' );
...
function learn_press_permission_view_quiz( $template ) {
	...
	if ( ! current_user_can( 'edit-lp_quiz' ) && ! $user->can_view_item( $quiz->id ) ) {
	...
	if ( $items = $course->get_curriculum_items() ) {
		foreach ( $items as $item ) {
			$item          = array(
				...
				'completed' => false,
				'viewable'  => $user->can_view_item( $item->ID ) !== false
			);
			$js['items'][] = $item;
		}
	}
...
if ( ! function_exists( 'learn_press_content_item_nav' ) ) {
	...
	function learn_press_content_item_nav() {
		...
		if ( $item_nav = $course->get_item_nav( false, true ) ) {
			if ( $item_nav[0] ) {
...
function learn_press_comments_template_query_args( $comment_args ) {
	$post_type = learn_press_get_post_type( $comment_args['post_id'] );
...
if ( ! function_exists( 'learn_press_filter_get_comments_number' ) ) {
	function learn_press_filter_get_comments_number( $count, $post_id = 0 ) {
		...
		if ( learn_press_get_post_type( $post_id ) == LP_COURSE_CPT ) {
			$sql = $wpdb->prepare(
				" SELECT count(*) "
				. " FROM {$wpdb->comments} "
 ...
 * @since 3.0.0
 *
 * @param int $course_id Added since 3.x.x
 *
 * @return bool
 */
function learn_press_current_user_enrolled_course( $course_id = 0 ) {
	$user = learn_press_get_current_user();
...
/**
 * Check if one user has enrolled to a course.
 *
 * @param int $user_id
 * @param int $course_id
 *
 * @return bool
 */
function learn_press_user_enrolled_course( $user_id = 0, $course_id = 0 ) {
...
function learn_press_filter_block_content_template( $located, $template_name, $args, $template_path, $default_path ) {
	...
	if ( $template_name == 'global/block-content.php' ) {
		...
		$located = learn_press_locate_template( 'single-course/content-protected.php', array( 'can_view_item' => $can_view_item ) );
	}
...
function learn_press_vm_before_content_items() {
	learn_press_get_template( '_vm/global/block-content' );
}