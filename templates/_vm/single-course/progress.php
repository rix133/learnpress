...
<div id="learn-press-course-progress" class="learn-press-course-results-progress">
    ...
    <div class="items-progress">
        ...
        <span class="number">{{getCompletedItemsHtml()}}</span>
        <div class="learn-press-progress lp-course-progress">
            <div class="progress-bg lp-progress-bar">
                <div class="progress-active lp-progress-value" :style="getProgressStyles()">
                </div>
            </div>
        </div>
    ...
    <div class="course-progress">
        ...
        <div class="lp-course-status">
            <span class="number">{{getResults(true)}}<span
        ...
        <div class="learn-press-progress lp-course-progress <?php echo $course_data->is_passed() ? ' passed' : ''; ?>"
            ...
            <div class="progress-bg lp-progress-bar">
                <div class="progress-active lp-progress-value" :style="{left: $courseStore().results}">
                </div>
            </div>
...
<script>
    jQuery(document).on('course-ready', function (e, data) {
        ...
        window.$vmCourseProgress = new Vue({
            ...
            methods: {
                getResults: function (sign) {
                    var r = this.$courseStore().results;

                    return sign ? parseFloat(r) : r;
                },
                getCompletedItemsHtml: function () {
                    return LP.l10n.translate('%d of %d items', this.$courseStore('countCompletedItems')(), this.$courseStore('countItems')())
                },
                getProgressStyles: function () {
                    console.log(this.getResults())
                    return {
                        left: this.getResults()
                    }
                },
